...
/**
 * Download rewritten content as a properly formatted .docx
 * This creates a clean document with proper Word styles
 * @param {string} text - The rewritten document text
 * @param {string} filename - Desired filename (without extension)
 */
export async function downloadRewrittenAsDocx(text, filename = 'document-rewritten') {
    ...
    while (i < lines.length) {
        ...
        if (type === 'tableRow') {
            const tableLines = [];
            while (i < lines.length && detectLineType(lines[i].trim()) === 'tableRow') {
